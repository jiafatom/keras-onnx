...
def _get_node_by_name_with_tf_layer_symbol(ts_op, node_list, tf_layer_symbol):
    cur_node = get_node_by_name(node_list, GRAPH_OUTMOST_NAME + '/' + ts_op.name, exact_match=True)
    if cur_node is not None:
        return cur_node
    else:
        return get_node_by_name(node_list, GRAPH_OUTMOST_NAME + '/' + tf_layer_symbol + ts_op.name, exact_match=True)
...
def _create_keras_nodelist(layer, node_list):
    ...
    for n_ in newly:
        cur_node = _get_node_by_name_with_tf_layer_symbol(n_, node_list, tf_layer_symbol)
        if cur_node is not None:
            output_nodes.append(cur_node)
    '''
    visited = set()
...
def _parse_graph_scope(graph, keras_op_table, topology, top_scope, target_opset, output_names):
    ...
    while not q_overall.empty():
        ...
        if node.name in keras_op_table:
            ...
            # tf node namy may add an additional layer, we extract it as tf_layer_symbol
            
            set_value = 0
            name_vector = node.name.split('/')
            if len(name_vector) > 2:
                node_name = name_vector[0] + '/' + '/'.join(name_vector[2:])
                tf_layer_symbol = name_vector[1] + '/'
                if node_name in keras_op_table:
                    type_k, curr_scope_name = \
                        (keras_op_table[node_name], GRAPH_OUTMOST_NAME + '/' + keras_op_table[node_name].name)
                    set_value = 1
            if set_value == 0:
                type_k, curr_scope_name = \
